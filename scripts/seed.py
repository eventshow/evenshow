...
CATEGORIES = ['Cocina', 'Juegos de mesa', 'Deporte', 'Otros']
ENROLLMENT_STATUS = ['ACCEPTED', 'PENDING', 'REJECTED']
EVENT_PKS_THIS_YEAR = range(1, 16)
EVENT_PKS_FUTURE = range(16, 31)
PROFILE_IMAGE_FILE = '/fixtures/avatar.png'
EVENT_IMAGE_FILE = '/fixtures/event.jpg'
FAKE = Faker('es_ES')
USER_PKS = range(1, 51)
...
def seed_events(start_day, event_pks):
    ...
    for event_pk in event_pks:
        ...
        fields = {
            ...
            'price': FAKE.random_number(1, 999999),
            'created_by': host,
            'category': category,
        }
        event = {
        ...
        seed_event_enrollments(event_pk, attendees, start_day)
...
def seed_event_ratings(event, reviewed, reviewers, created_at):
    for reviewer in reviewers:
        on = random.choice(['ATTENDEE', 'HOST'])
        created_at_ = FAKE.date_time_between(
            start_date=created_at, end_date='+1y').strftime('%Y-%m-%d %H:%M:%S%z')
        ...
        fields = {
            ...
            'event': event,
            'on': on,
            'created_at': created_at_ + '+0000',
            'updated_at': created_at_ + '+0000',
            'created_by': reviewer,
            'reviewed': reviewed,
        }
        rating = {
...
def seed_event_enrollments(event, attendees, created_at):
    created_at_ = FAKE.date_time_between(
        start_date='-1y', end_date=created_at).strftime('%Y-%m-%d %H:%M:%S%z')
